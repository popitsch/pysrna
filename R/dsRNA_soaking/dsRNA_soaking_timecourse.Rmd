...
params:
    sample_sheet:
      value: "/Volumes/groups/ameres/Niko/projects/Ameres/nina/dsRNA_soaking_timecourse/sample_sheet.tsv"
    ref:
      value: "/Volumes/groups/ameres/Niko/projects/Ameres/nina/dsRNA_soaking_timecourse/ref/"
    results:
      value: "/Volumes/groups/ameres/Niko/projects/Ameres/nina/dsRNA_soaking_timecourse/results/"
    out_dir:
       value: "/Volumes/groups/ameres/Niko/projects/Ameres/nina/dsRNA_soaking_timecourse/analysis/"
    thomas_data:   
       value: "/Volumes/groups/bioinfo/shared/Ameres.GRP/Nina.Khaldieh/R14043/"
---
...
#
plot_corr = function(data, a, b, c=1, labx=a, laby=b, main=paste0("Correlation between ", a, " and ", b), drawdiag=F, log=F, limits0=T, removeOutliers=F, testCorrelation=F, cor_y_pos=0.85, binhex=T, direct_labels=F, return_plot=F) {
  if (testCorrelation) {
  ...
  if ( binhex ) {
    ...
    p = p + geom_point(aes_string(col=c)) 
    if ( ! direct_labels) {
...
# Data
```{r}
# NOTE: pooled data ignored for now as also not in Tomas' data
sample_sheet = load_table(params$sample_sheet) %>% filter(sample_name != 'pooled_iii')
sample_names = unique(sample_sheet$sample_name)
sample_sheet=sample_sheet %>% 
  mutate(sample_name=factor(sample_name, levels=!!sample_names))
# add raw read counts (from demux stats)
sample_sheet = sample_sheet %>%  left_join(
  load_table('/Volumes/groups/ameres/Niko/projects/Ameres/nina/dsRNA_soaking_timecourse/01_raw/raw_read_counts.tsv') %>% 
    select(sample_id=id, raw_reads=count), by='sample_id') 
...
for ( sample in sample_sheet %>% pull(sample_name)) {
  fn=paste0(sample_sheet %>% filter(sample_name==sample) %>% pull(fn), '.fastq')
  parsed_read_stats[[sample]] = load_table(paste0(params$results, '/parsed_reads/',fn, '.stats.tsv.gz'))
  sc[[sample]] = load_table(paste0(params$results, '/spikein_counts/', fn, '_trimmed.fq.counts_spikein.tsv'))
  fc[[sample]] = load_table(paste0(params$results, '/feature_counts/', fn, '.featureCounts.txt')) %>% dplyr::rename(count=8)
  fc_stats[[sample]] = load_table(paste0(params$results, '/feature_counts/', fn, '.featureCounts.txt.summary'), header = F) %>% 
    filter(V1 != "Status") %>% 
...
fc = fc %>% 
  bind_rows(.id='sample_name') %>% 
  mutate(sample_name=factor(sample_name, levels=!!sample_names)) %>% 
  left_join(load_table(paste0(params$results, '/transcriptome/dsRNA_soaking_timecourse.feature_meta.tsv')), by='Geneid') %>% 
...
sc = sc %>% 
  bind_rows(.id='sample_name') %>% 
  mutate(sample_name=factor(sample_name, levels=!!sample_names)) %>% 
  left_join(load_table(paste0(params$ref, '/spikeins_meta.tsv')), by='si_name') %>% 
